...
Save to: `.opencode/handoffs/HANDOFF.md`
...
- `path/to/file` — [what changed and why, 1 line]
- `path/to/new-file` — [NEW: what this file does]
...
- **Validation:** [clean/issues — what kind]
...
2. Suggest the next session command:
   ```
   Read `.opencode/handoffs/HANDOFF.md` and continue from where the previous session left off.
   ```
...
- Don't be vague ("fix the bug") — be specific ("fix the validation in `src/auth/token.ts` by adding expiry check after the refresh call")
